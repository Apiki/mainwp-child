...
class MainWP_Child
{
    ...
    public function __construct($plugin_file)
    {
        ...
        add_action('init', [&$this, 'init_check_login'], 1);
        ...
        add_action('core_upgrade_preamble', [MainWP_Child_Updates::get_instance(), 'detect_premium_themesplugins_updates']);
        ...
        /**
         * Apiki: Added to support the MainWP Child plugin to integrate with external repositories via MainWP Dashboard.
         */
        MainWP_Repository::instance()->init();
        ...
        if (defined('DOING_CRON') && DOING_CRON) {
            if (isset($_GET['mainwp_child_run']) && ! empty($_GET['mainwp_child_run'])) { // phpcs:ignore WordPress.Security.NonceVerification
                add_action('init', [MainWP_Utility::get_class_name(), 'cron_active'], PHP_INT_MAX);
            }
        }
         ...
         * @since 4.3
         */
        add_action('mainwp_child_write', [MainWP_Helper::class, 'write']);

        add_filter('mainwp_child_create_action_nonce', [MainWP_Utility::class, 'hook_create_nonce_action'], 10, 2);
        add_filter('mainwp_child_verify_authed_acion_nonce', [MainWP_Utility::class, 'hook_verify_authed_action_nonce'], 10, 2);
        add_filter('mainwp_child_get_ping_nonce', [MainWP_Utility::class, 'hook_get_ping_nonce'], 10, 2);
        add_filter('mainwp_child_get_encrypted_option', [MainWP_Child_Keys_Manager::class, 'hook_get_encrypted_option'], 10, 3);
    }
    ...
    public function parse_init()
    {
        ...
        if (null === $_wp_submenu_nopriv) {
            $_wp_submenu_nopriv = []; // phpcs:ignore -- Required to fix warnings, pull request solutions appreciated.
        }
    ...
    public function activation()
    {
        ...
        $to_delete = ['mainwp_ext_snippets_enabled', 'mainwp_ext_code_snippets'];
        foreach ($to_delete as $delete) {
            delete_option($delete);
        }
    }
