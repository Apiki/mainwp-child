...
class MainWP_Child_DB_Updater
{
    ...
    {
        add_filter('mainwp_site_sync_others_data', [$this, 'sync_others_data'], 10, 2);
     ...
     */
    public function sync_others_data($information, $data = [])
    {
        if (isset($data['syncDBUpdater']) && $data['syncDBUpdater']) {
            try {
                $information['syncDBUpdaterResponse'] = [
    ...
    {
        $sync_plugin_db_upgrades = apply_filters('mainwp_child_db_updater_sync_data', []);
    ...
    public function update_db()
    {
        ...
        $plugins = isset($_POST['list']) ? explode(',',
            urldecode(wp_unslash($_POST['list']))) : []; // phpcs:ignore WordPress.Security.NonceVerification,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        // phpcs:enable
        $upgrades = [];
