...
class MainWP_Pages {
    ...
    public function admin_notice() { //phpcs:ignore -- NOSONAR -complexity.
        ...
        if (! get_option('mainwp_child_pubkey') && MainWP_Helper::is_admin() && is_admin()) {
            ...
            if (! MainWP_Child_Branding::instance()->is_branding()) {
                $msg .= '<div style="font-size:1.2em;">' . esc_html__( 'For additional security options, visit the ', 'mainwp-child' ) . esc_html( $child_name ) . sprintf( esc_html__( ' %1$splugin settings%2$s. ', 'maiwnip-child' ), '<a href="admin.php?page=wpdash_child_tab">', '</a>' ) . '</div>';
        ...
        if ( isset( $_GET['page'] ) && 'wpdash_child_tab' === $_GET['page'] && isset( $_GET['message'] ) ) { //phpcs:ignore -- ok.
    ...
    public function admin_init() { //phpcs:ignore -- NOSONAR - complex method.
        ...
        if ( isset( $_POST['nonce-disconnect'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce-disconnect'] ) ), 'child-settings-disconnect' ) ) {
            ...
            wp_safe_redirect( 'options-general.php?page=wpdash_child_tab&message=1' );
        }
        ...
        if ( isset( $_POST['submit'] ) && isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'child-settings' ) ) {
            ...
            MainWP_Helper::update_option( 'mainwp_child_ttl_active_unconnected_site', ! empty( $_POST['mainwp_child_active_time_for_unconnected_site'] ) ? (int) $_POST['mainwp_child_active_time_for_unconnected_site'] : 0 );
            ...
            wp_safe_redirect( 'options-general.php?page=wpdash_child_tab&message=2' );
        }
        // phpcs:enable
    }
    ...
    private function init_pages( $child_menu_title, $child_page_title ) { //phpcs:ignore -- NOSONAR - complex.
        ...
        $settingsPage = add_submenu_page( 'options-general.php', $child_page_title, $child_menu_title, 'manage_options', 'wpdash_child_tab', array( &$this, 'render_pages' ) );
    ...
    public static function render_header( $shownPage, $subpage = true, &$show_clone_funcs = true ) { // phpcs:ignore -- NOSONAR - Current complexity is the only way to achieve desired results, pull request solutions appreciated.
        ...
        <div class="" id="mainwp-child-settings-page">
            <h1><?php echo esc_html( null === static::$brandingTitle ? 'WPDash Child' : static::$brandingTitle ); ?></h1>
            <div class="" id="mainwp-child-settings-page-content">
                <div class="" id="mainwp-child-settings-page-navigation">
                    <?php if ( ! $hide_settings ) : ?>
                        <a class="nav-tab pos-nav-tab <?php echo ( 'settings' === $shownPage ) ? 'nav-tab-active' : ''; ?>" tab-slug="settings" href="<?php echo $subpage ? 'options-general.php?page=wpdash_child_tab&tab=settings' : '#'; ?>"><?php esc_html_e( 'Settings', 'mainwp-child' ); ?></a>
                    <?php endif; ?>
                    <?php if ( ! $hide_restore && $show_clone_funcs ) : ?>
                        <a class="nav-tab pos-nav-tab <?php echo ( 'restore-clone' === $shownPage ) ? 'nav-tab-active' : ''; ?>" tab-slug="restore-clone" href="<?php echo $subpage ? 'options-general.php?page=wpdash_child_tab&tab=restore-clone' : '#'; ?>"><?php echo esc_html__( 0 !== (int) $sitesToClone ? 'Restore / Clone' : 'Restore', 'mainwp-child' ); ?></a>
                    <?php endif; ?>
                    <?php if ( ! $hide_server_info ) : ?>
                        <a class="nav-tab pos-nav-tab <?php echo ( 'server-info' === $shownPage ) ? 'nav-tab-active' : ''; ?>" tab-slug="server-info" href="<?php echo $subpage ? 'options-general.php?page=mwpdash_child_tab&tab=server-info' : '#'; ?>"><?php esc_html_e( 'Server Information', 'mainwp-child' ); ?></a>
                    <?php endif; ?>
                    <?php if ( ! $hide_connection_detail ) : ?>
                        <a class="nav-tab pos-nav-tab <?php echo ( 'connection-detail' === $shownPage ) ? 'nav-tab-active' : ''; ?>" tab-slug="connection-detail" href="<?php echo $subpage ? 'options-general.php?page=wpdash_child_tab&tab=connection-detail' : '#'; ?>"><?php esc_html_e( 'Connection Details', 'mainwp-child' ); ?></a>
                    <?php endif; ?>
    ...
    public function render_settings()
    {
        ...
        <br/>
            ...
            <p><?php esc_html_e( $branding_title . ' requests that you connect using an admin account and password for the initial setup. Rest assured, your password is never stored by your Dashboard and never sent to ' . strtolower($branding_title) . '.apiki.com. Once this initial connection is complete, your ' . $branding_title . ' Dashboard generates a secure Public and Private key pair (2048 bits) using OpenSSL, allowing future connections without needing your password again. For added security, you can even change this admin password once connected just be sure not to delete the admin account, as this would disrupt the connection.', 'mainwp-child' ); ?></p>
        ...
        <form method="post" onsubmit="return confirm('<?php echo esc_js( __( 'Are you sure you want to Disconnect Site from your ' . $branding_title . ' Dashboard?', 'mainwp-child' ) ); ?>');"  action="options-general.php?page=wpdash_child_tab">
